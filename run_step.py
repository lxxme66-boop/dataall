...
class ModularQAPipeline:
    ...
    def __init__(self, config: dict = None):
        ...
        # 步骤状态跟踪
        self.step_status = {
            "1.1": {"completed": False, "output": None},
            "1.2": {"completed": False, "output": None},
            "1.3": {"completed": False, "output": None},
            "2.1": {"completed": False, "output": None},
            "2.2": {"completed": False, "output": None},
            "2.3": {"completed": False, "output": None},
            "2.4": {"completed": False, "output": None},
            "3.0": {"completed": False, "output": None}
        }
    ...
    def save_step_status(self):
        """保存步骤状态"""
        status_file = os.path.join(self.output_dir, "step_status.json")
        with open(status_file, 'w', encoding='utf-8') as f:
            json.dump(self.step_status, f, ensure_ascii=False, indent=2)
    ...
    async def step_1_2_ai_processing(self, input_tasks=None, batch_size: int = 2):
        """步骤1.2: AI文本处理"""
        ...
        # 执行AI处理
        processed_results = []
        total_batches = (len(input_tasks) + batch_size - 1) // batch_size
        
        for i in range(0, len(input_tasks), batch_size):
            batch = input_tasks[i:i+batch_size]
            ...
            batch_results = await asyncio.gather(
                *(input_text_process(
                    ...
                    total_chunks=len([t for t in input_tasks if t["file_path"] == task["file_path"]]),
                    prompt_index=9,
                    config=self.config
                ) for task in batch),
...
async def main():
    """主函数 - 支持单步骤运行"""
    parser = argparse.ArgumentParser(description="半导体QA生成系统 - 模块化版本")
    ...
    # 初始化流水线
    pipeline = ModularQAPipeline(config)
    ...
    try:
        ...
        if args.step == "status":
            ...
            await pipeline.step_1_2_ai_processing(batch_size=args.batch_size)
        elif args.step == "1.3":
